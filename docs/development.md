...
项目已发布 `v0.2.1` Alpha，当前处于 0.2.x 跨平台稳定化阶段。已落地能力如下：
...
- MongoDB Shell/BSON 使用独立的非执行转换器：完整输入必须是 JSON 结构，只替换
  ...
  `NumberInt`/`NumberLong`/`Long` 还支持直接整数，但表达式会被拒绝；转换结果复用 JSON 高亮、折叠、搜索与操作。
  对象键可使用 ASCII 标识符形式，普通值可使用单引号字符串；转换完成后仍交给严格
  JSON 解析器验证完整结构，不支持变量引用、运算符或任意函数调用。
  `/pattern/options` 正则字面量只做词法转换，不编译或执行；模式转义按 BSON 语义保留，
  选项限制为 `i/m/s/u/x` 并排序，重复项和 `g` 会返回稳定错误码。
  `BSONRegExp` 同样只做词法转换，接受 BSON 的 `i/l/m/s/u/x` 选项并排序。
  `HexData` 要求完整偶数位十六进制字节，不沿用 mongosh 遇到非法字符时静默截断的行为；
...
- JSON 对象/数组、XML 多行元素和 YAML 缩进块支持行号槽折叠，使用带边框的小方形 `-`（展开）和 `+`（收起）控件；结果区获得焦点后，左/右方向键可收起/展开当前视口的第一个可折叠节点。折叠只改变可见行映射，不修改格式化结果。搜索跳转会展开包含当前命中的折叠区间，全选与复制仍基于完整原文。
...
- CRC32、MD5、SHA-1、SHA-256、SHA-384、SHA-512 与 HMAC 已完成，支持 Hex/Base64 输出；
...
- Base64 二进制安全摘要（字节数与 Hex 预览），避免把二进制直接渲染为乱码；
  ...
  WAV、PDF、ZIP、Gzip、7-Zip、RAR 和 SQLite 会按魔数显示类型并建议匹配扩展名，
  未知格式安全回退为 `.bin`。
- Base64 解码和 HMAC Base64 密钥均接受标准 `+/` 或 URL-safe `-_` 字母表，
  但拒绝在同一输入内混用两套字母表；空白、合法省略填充与尾部位仍按规范校验。
- Base64 Data URL 仅在元数据最后一个标记为 `;base64` 时识别，文本和二进制均只
  自动解码一层；非 Base64 Data URL 保持普通文本，二进制继续使用魔数预览与导出。
...
页内编辑、选择与撤销已接入 EUI 原生输入模型；分页边界可继续选择，并以全局范围执行
复制、剪切、替换与全选。Windows 10 原生复验仍未完成，因此完整 UI 端到端验收保持
未完成。整机峰值采样与 Windows 11 原生验证结果见下文。

2026-08-27 在 macOS 开发机使用 UTF-8 JSON 验证中文、日文、韩文、emoji 和分解形式
`e + U+0301` 的文件输入、解析、高亮及字体回退；同时验证全选、复制、剪切、粘贴、
撤销和重做。系统剪贴板逐字节检查保留原始 UTF-8。自动化字符注入不经过 macOS 输入法
候选窗口，因此该结果不代表候选词组合态已验收，后者仍需人工操作系统输入法完成。
...
`detection-v1` 当前包含 57 个确定性回归样例，其中 31 个是可识别格式/编码、25 个是
纯文本负样例、1 个是应保留 XML 类型的无效结构。当前报告结果为：精确匹配
57/57、可识别内容召回率 100%、纯文本特异度 100%、无效输入错误保留率 100%。
...
CSV 大数据虚拟滚动、精确搜索定位以及 XML、YAML、摘要/HMAC、Base64、URL Decode 已完成首个可用版本。`v0.2.0` 提供 macOS arm64 DMG 和通过 MinGW-w64 交叉构建的 Windows x64 Portable ZIP，并在两个包中加入 CLI。Windows 11 原生 MinGW/MSVC 构建、启动和 IME 已完成；大文本分页边界连续选择、Tab/Shift+Tab 焦点遍历、按钮键盘激活、下拉框键盘选择与 CSV 方向键导航已接入。下拉框关闭和切换选项后会触发完整重绘，避免弹层残影。macOS AX 桥接已接入，Windows 10、VoiceOver 连续导航与签名/公证仍按发布清单推进。
...
同日评估 macOS Accessibility/AX 桥接：当时 EUI 元素模型只有布局、绘制和交互回调，
尚无语义角色、可访问名称/值或原生动作描述。应用层叠加透明原生控件会复制焦点、
命中和状态同步逻辑，因此没有采用该临时方案。后续实现改为从 DSL 实际布局发布原生
语义代理，并把原生动作送回 EUI 运行时的单一焦点与回调路径。

随后使用系统已安装的“简体拼音”逐键输入 `nihao`，输入区先保持未提交的
`ni'hao` 预编辑文本且结果区为空；按空格选择首个候选后正确提交“你好”，结果区随即
更新。该过程验证了 EUI 文本输入的 macOS IME 组合态、候选选择和提交路径。系统候选
面板属于独立窗口，当前应用级自动化截图不会包含它，因此面板相对插入光标的几何位置
仍保留为人工全屏视觉检查项。

2026-09-04 又使用当时实际选中的搜狗拼音，在窗口与全屏模式分别通过原生逐键事件输入
`ni'hao`。两次预编辑期间，AX 输入值及结果值都保持为空；按空格后均提交为“你好”，
结果区同步显示“你好”。这次复验排除了直接文本注入绕过输入法的假阳性，但应用级截图
仍无法捕获输入法的独立候选窗口，因此候选面板相对插入光标的位置没有被标记为已完成。

同次验收实际开启 macOS VoiceOver：当时的 AX 树只暴露窗口、标题栏交通灯、窗口标题
和菜单，连续导航无法进入画布内容。2026-09-03 随后加入原生 AX 语义桥接：应用每次
组合后从实际 DSL 布局发布稳定的 `NSAccessibilityElement`，AX 聚焦和按下动作回到 EUI
已有的焦点与点击回调，不叠加透明原生控件。实测 AX 树已包含本地化的输入/搜索文本框、
按钮、下拉框和只读结果文本区；通过 AX 聚焦输入并键入 JSON 后，格式化结果值立即可读，
“压缩”按钮也能执行。文本变化只发送 value 通知，结构或尺寸变化才发送 layout 通知，
避免实时格式化时打断屏幕阅读器当前位置。仍需实际开启 VoiceOver 做最终连续导航复验。
