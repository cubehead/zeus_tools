...
2026-09-04 又在窗口与全屏模式分别使用原生逐键事件输入 `ni'hao`。两次预编辑期间，
AX 输入值及结果值都保持为空；按空格后均提交为“你好”，结果区同步显示“你好”。这次
复验排除了直接文本注入绕过输入法的假阳性，但应用级截图仍无法捕获系统独立候选窗口，
因此候选面板相对插入光标的位置没有被标记为已完成。
