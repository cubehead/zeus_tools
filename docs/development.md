...
- 1 MiB 以上输入使用轻量源内容提示，避开 EUI 编辑器对超长单行的全量文本测量；
  后台处理、搜索模型、整份复制和导出仍保留全部字节。只读结果对单行最多测量/绘制
  16 KiB 可见前缀，防止病理长行阻塞 GPU 首屏，底层文档不截断。
...
（约 224 MiB）。该场景使用轻量输入提示和受限长行绘制，完整结果仍可复制/导出。
清空已由普通输入路径覆盖；大输入直接编辑、选择和撤销仍需虚拟化可编辑控件，因此
完整 UI 端到端验收保持未完成。
