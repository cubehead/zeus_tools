...
- MongoDB Shell/BSON 使用独立的非执行转换器：完整输入必须是 JSON 结构，只替换
  ...
  `ObjectId`、`ISODate`/`Date`、`UUID`、`Timestamp`、`BinData`、`HexData`、`BSONRegExp`、单参数 `Code`、
...
页内编辑、选择与撤销已接入 EUI 原生输入模型；跨页连续选择和 Windows 10 原生复验
仍未完成，因此完整 UI 端到端验收保持未完成。整机峰值采样与 Windows 11 原生验证
结果见下文。
