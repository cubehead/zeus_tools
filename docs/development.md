...
同次验收实际开启 macOS VoiceOver：AX 树只暴露窗口、标题栏交通灯、窗口标题和菜单；
使用 VoiceOver 进入窗口并连续导航仍停留在窗口容器，无法到达输入编辑器、操作按钮、
结果区或搜索控件。测试结束后已关闭 VoiceOver。该结果确认当前键盘导航可用，但屏幕
阅读器验收不通过，不能在发布说明中宣称辅助功能完整支持。
