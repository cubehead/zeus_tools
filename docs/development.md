...
| --- | ---: | ---: | ---: | ---: |
| JSON | 163,029 个对象 | 1309 ms | 76 ms | 163,029 |
| CSV | 151,384 条数据行、5 列 | 1116 ms | 139 ms | 151,384 |

两个场景在同一进程顺序执行时最大常驻内存约 299 MiB。CSV 结果只建立虚拟表格模型，
不再重复创建同体量的纯文本高亮文档。该基准已经覆盖完整后台处理服务，但仍不包含
EUI 输入控件、GPU 首屏、滚动、清空和撤销，因此 UI 端到端验收仍保持未完成。
