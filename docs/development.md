...
| CSV | 151,384 条数据行、5 列 | 1070 ms | 144 ms | 151,384 |
...
高亮文档。该基准已经覆盖完整后台处理服务。另有 10 MiB + 1 byte 文档场景验证
...
合成数据的固定场景：`json`、`csv`、`jwt`、`hmac`、`about`、`large-json` 或
`oversized`。其中后两项分别生成精确 10 MiB JSON 和 10 MiB + 1 byte 文本，仅用于
性能/保护边界验收。该值未设置时，合成数据
不会编译进应用，也不会改变正常发行行为。
