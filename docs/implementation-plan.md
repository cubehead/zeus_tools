...
| P1 | 键盘与可访问性 | 已完成 Tab/Shift+Tab 循环焦点、可见焦点框、Enter/Space 按钮激活、下拉框键盘选择、搜索聚焦、Esc 关闭、CSV 方向键导航、只读文本键盘滚动及结构化结果左右键折叠。2026-08-31 实际 VoiceOver 验收不通过：只能识别窗口与标题栏，无法进入应用内容。当前 EUI 元素模型没有语义角色、可访问名称/值或原生动作接口；需先在框架层设计语义快照与原生桥接，避免用透明控件复制第二套焦点和命中逻辑。 |
