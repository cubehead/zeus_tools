...
- MongoDB Shell 中白名单内的 `NumberInt`/`Int32`、`NumberLong`/`Long`、`Double`、
...
- MongoDB 正则字面量转换为 Extended JSON v2 `$regularExpression`；模式不会执行，
  选项只允许 `i/m/s/u/x`、去重并按字母序输出，`g` 明确拒绝。
...
- C++17 核心测试和精确 1 MB/10 MiB JSON、CSV 基准程序；10 MiB 应用处理服务基准覆盖
...
- 自动检测报告覆盖 75 个版本化样例；当前精确匹配率、可识别内容召回率和纯文本
...
| P0 | 剩余输入与 IME 验证 | Windows 11 中文 IME、emoji 与组合字符已通过；macOS 已验证 UTF-8 文件输入、中文/日文/韩文、emoji、组合字符、全选、复制、剪切、粘贴、撤销和重做。系统简体拼音的预编辑、首候选选择和中文提交已通过；2026-09-04 又以搜狗拼音完成窗口与全屏复验，组合提交前结果保持为空。候选面板相对插入光标的几何位置仍需人工全屏视觉确认，并需完成 Windows 10 复验。 |
...
| P1 | 键盘与可访问性 | 已完成 Tab/Shift+Tab 循环焦点、可见焦点框、Enter/Space 按钮激活、下拉框键盘选择、搜索聚焦、Esc 关闭、CSV 方向键导航、只读文本键盘滚动及结构化结果左右键折叠。2026-09-03 已加入原生 macOS AX 语义桥接：AX 树可识别本地化输入/搜索文本框、结果文本区、按钮和下拉框；AX 聚焦输入、键入 JSON、朗读格式化结果及激活“压缩”动作均已通过。桥接复用 EUI 的真实焦点和回调，没有叠加透明控件；仍需开启 VoiceOver 做最终连续导航复验。 |
...
| 0.2.x 稳定化 | 进行中 | 完成 Windows 10、macOS/Windows 10 IME 及键盘可访问性验收。 |
