...
- 支持 JSON 转义/反转义，并且只自动解码一层 Base64 或 URL 编码。
...
| 摘要/HMAC | 手动在本地计算 | MD5、SHA-1/256/512、Hex 和 Base64 输出 |
...
- YAML 有意只接受单文档安全子集，并拒绝指令、标签、锚点、别名和多文档
  输入。
- JWT 检查会解码声明，但不会验证签名。
- Base64/URL 自动解码在一层后停止。目前尚无专用的“继续解码”操作；如需
  手动处理下一层，请将结果复制回输入框。
- HMAC 当前使用原始输入，并同时显示 Hex 和 Base64。消息来源选择、密钥编码
  选择和密钥遮罩输入框仍在规划中；请勿在 Alpha 演示中使用真实敏感密钥。
